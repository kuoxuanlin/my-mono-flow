...
import io  # 導入 IO 模組來處理導出問題

# --- 0. 基礎配置 ---
DB_FILE = "mono_v13_data.json"
...
n_name = c1.text_input("任務", placeholder="...", key="new_task", label_visibility="collapsed")
...
if page == "儀錶板":
    ...
    output.write("import json, os, time\\n")
    output.write("import pandas as pd\\n")
    output.write("import plotly.express as px\\n")
    output.write("from datetime import datetime, timedelta\\n\\n")
    output.write(f"DB_FILE = '{DB_FILE}'\\n")
    ...
    # 轉換為 UTF-8 格式的位元組
    processed_code = output.getvalue().encode('utf-8')
